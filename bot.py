...
# --------------------------------------------
# PERSISTENT SESSION WITH IMPERSONATION
# --------------------------------------------
...
# --------------------------------------------
def get_cached_channel_history(channel_id, before_id):
    now = time.time()
...
# CONTEXT BUILDER
# --------------------------------------------
def build_context(channel_id, current_msg, guild_id=None):
    lines = []
    channel_msgs = get_cached_channel_history(channel_id, current_msg["id"])
...
def send_typing(channel_id):
    api_request("POST", f"https://discord.com/api/v9/channels/{channel_id}/typing")

def send_reply(channel_id, reply_to_id, content):
    url = f"https://discord.com/api/v9/channels/{channel_id}/messages"
    payload = {
        "content": content,
...
async def handle_message(msg):
    ...
    context = build_context(channel_id, msg, guild_id)
    ...
    send_typing(channel_id)
    log.info("Waiting 5 seconds (typing) before sending reply...")
    await asyncio.sleep(5.0)

    for i in range(0, len(reply_text), 1900):
        chunk = reply_text[i:i+1900]
        send_reply(channel_id, msg_id, chunk)
...
async def filter_and_queue(msg):
    ...
    if mentioned or replied:
        log.info(f"Server ping from {msg['author']['username']} -> queue")
        await message_queue.put(msg)
...
async def listen():
    ...
    gw = api_request("GET", "https://discord.com/api/v9/gateway").json()["url"]
...
async def main():
    global self_user_id
    resp = api_request("GET", "https://discord.com/api/v9/users/@me")
    if resp.status_code != 200:
